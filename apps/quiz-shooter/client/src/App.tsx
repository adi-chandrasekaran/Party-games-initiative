...
export default function App() {
  ...
  const [selectedSharedDeckId, setSelectedSharedDeckId] = useState("");
  ...
  useEffect(() => {
    platformRequest("/api/decks")
      .then((payload) => {
        ...
        if (fromUrl) {
          setSelectedSharedDeckId(fromUrl);
        }
      })
      .catch(() => null);
  }, []);
  ...
  useEffect(() => {
    if (!selectedSharedDeckId) return;
    const deckMatch = sharedDecks.find((entry) => entry.id === selectedSharedDeckId);
    if (!deckMatch?.text) return;
    const nextDeck = parseDeck(deckMatch.text);
    if (nextDeck.length) {
      setDeck(nextDeck);
      setToast(`Loaded shared deck: ${deckMatch.title}`);
    }
  }, [selectedSharedDeckId, sharedDecks]);
  ...
  async function createRoom() {
    try {
      ...
      const access = await platformRequest("/api/platform/can-host", {
        method: "POST",
        body: { gameId: "quiz-shooter", emailOrUsername: hostContact.trim() },
      });
      ...
      socket.emit("create-room", { hostName: hostContact || "Host", deck }, (response: any) => {
        if (!response.ok) {
  ...
  function applySharedDeck() {
    const deckMatch = sharedDecks.find((entry) => entry.id === selectedSharedDeckId);
    if (!deckMatch?.text) {
      setToast("That deck does not have extracted text yet.");
      return;
    }

    const nextDeck = parseDeck(deckMatch.text);
    if (!nextDeck.length) {
      setToast("That deck did not contain any questions.");
      return;
    }
        ...
        <section className="panel">
            ...
            <div className="form-stack">
              ...
              {sharedDecks.length > 0 && (
                <>
                  <select
                    className="select"
                    value={selectedSharedDeckId}
                    onChange={(event) => setSelectedSharedDeckId(event.target.value)}
                  >
                    <option value="">Select a shared deck</option>
                    {sharedDecks.map((sharedDeck) => (
                      <option key={sharedDeck.id} value={sharedDeck.id}>
                        {sharedDeck.title}
                      </option>
                    ))}
                  </select>
                  <button className="secondary-btn" onClick={applySharedDeck} disabled={!selectedSharedDeckId}>
                    Use shared deck
                  </button>
                </>
              )}
          ...
          {mode === "lobby" && (
            <div className="form-stack">
              ...
              <p className="muted">
                You joined as <strong>{hostContact || "Player"}</strong> on the{" "}
