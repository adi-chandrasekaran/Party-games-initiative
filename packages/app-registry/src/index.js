...
export const appManifests = Object.freeze([
  { id: "imposter", title: "IMPOSTER", subtitle: "Find the secret player", area: "arcade", canonicalRoute: "/arcade/imposter", launchMode: "legacy-external", access: "public-host", deckCapability: "optional", color: "#ff2fa3", icon: "imposter", packageName: "imposter-game", apiContracts: ["platformGames", "gameAccess", "canHost"], legacyTarget: legacyTarget("VITE_IMPOSTER_ORIGIN", "http://localhost:5181") },
  ...
  { id: "habit-tracker", title: "HABIT TRACKER", subtitle: "Month tabs, daily checkboxes", area: "planner", canonicalRoute: "/planner/habit-tracker", launchMode: "legacy-external", access: "authenticated-member", deckCapability: "none", color: "#4dd6ff", icon: "habit", packageName: "planner-habit-tracker", apiContracts: [], legacyTarget: legacyTarget("VITE_HABIT_TRACKER_ORIGIN", "http://localhost:5314") },
  { id: "todo-board", title: "TO-DO BOARD", subtitle: "Priority tabs and task lanes", area: "planner", canonicalRoute: "/planner/todo-board", launchMode: "legacy-external", access: "authenticated-member", deckCapability: "none", color: "#ff8a3d", icon: "todo", packageName: "planner-todo-board", apiContracts: [], legacyTarget: legacyTarget("VITE_TODO_BOARD_ORIGIN", "http://localhost:5315") },
  { id: "timer", title: "TIMER", subtitle: "Six study timers in one app", area: "planner", canonicalRoute: "/planner/timer", launchMode: "legacy-external", access: "authenticated-member", deckCapability: "none", color: "#7cff2f", icon: "timer", packageName: "planner-timer", apiContracts: [], legacyTarget: legacyTarget("VITE_TIMER_ORIGIN", "http://localhost:5316") },
  { id: "assignments", title: "ASSIGNMENTS", subtitle: "Spreadsheet-style tracker", area: "planner", canonicalRoute: "/planner/assignments", launchMode: "legacy-external", access: "authenticated-member", deckCapability: "none", color: "#ff59c7", icon: "assignments", packageName: "planner-assignments", apiContracts: [], legacyTarget: legacyTarget("VITE_ASSIGNMENTS_ORIGIN", "http://localhost:5317") },
]);
...
export function validateAppRegistry(manifests = appManifests) {
  ...
  for (const manifest of manifests) {
    ...
    if (manifest.launchMode === "legacy-external") {
      ...
      assertContract(!manifest.legacyTarget, `Embedded app ${manifest.id} cannot declare a legacy target.`);
    }
  }
  return manifests;
}
...
export function resolveLegacyLaunch(manifestOrId, environment = {}) {
  ...
  assertContract(manifest.launchMode === "legacy-external", `${manifest.id} is not a legacy launcher.`);
  const override = environment[manifest.legacyTarget.environmentKey];
  return assertLegacyOrigin(override || manifest.legacyTarget.defaultOrigin, manifest.id);
}
...
export function toLauncherCard(manifestOrId, environment = {}) {
  ...
  return Object.freeze({
    ...manifest,
    legacyUrl: manifest.launchMode === "legacy-external" ? resolveLegacyLaunch(manifest, environment) : null,
  });
}
