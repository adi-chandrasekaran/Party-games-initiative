...
export const appManifests = Object.freeze([
  { id: "imposter", title: "IMPOSTER", subtitle: "Find the secret player", area: "arcade", canonicalRoute: "/arcade/imposter", launchMode: "same-origin", sameOriginEntry: "/microapps/imposter/index.html", access: "public-host", deckCapability: "optional", color: "#ff2fa3", icon: "imposter", packageName: "imposter-game", apiContracts: ["platformGames", "gameAccess", "canHost"] },
  { id: "quiz-shooter", title: "QUIZ SHOOTER", subtitle: "Answer fast, shoot faster", area: "arcade", canonicalRoute: "/arcade/quiz-shooter", launchMode: "same-origin", sameOriginEntry: "/microapps/quiz-shooter/index.html", access: "public-host", deckCapability: "optional", color: "#18d8ff", icon: "quiz", packageName: "quiz-shooter-3d-client", apiContracts: ["platformGames", "gameAccess", "canHost"], realtimeContract: "quizShooter" },
  { id: "build-a-beast", title: "BUILD A BEAST", subtitle: "Create your monster", area: "arcade", canonicalRoute: "/arcade/build-a-beast", launchMode: "same-origin", sameOriginEntry: "/microapps/build-a-beast/index.html", access: "public-host", deckCapability: "optional", color: "#7cff2f", icon: "beast", packageName: "build-a-beast-client", apiContracts: ["platformGames", "gameAccess", "canHost"], realtimeContract: "buildABeast" },
  ...
  { id: "habit-tracker", title: "HABIT TRACKER", subtitle: "Month tabs, daily checkboxes", area: "planner", canonicalRoute: "/planner/habit-tracker", launchMode: "same-origin", sameOriginEntry: "/microapps/habit-tracker/index.html", access: "authenticated-member", deckCapability: "none", color: "#4dd6ff", icon: "habit", packageName: "planner-habit-tracker", apiContracts: [] },
  { id: "todo-board", title: "TO-DO BOARD", subtitle: "Priority tabs and task lanes", area: "planner", canonicalRoute: "/planner/todo-board", launchMode: "same-origin", sameOriginEntry: "/microapps/todo-board/index.html", access: "authenticated-member", deckCapability: "none", color: "#ff8a3d", icon: "todo", packageName: "planner-todo-board", apiContracts: [] },
  { id: "timer", title: "TIMER", subtitle: "Six study timers in one app", area: "planner", canonicalRoute: "/planner/timer", launchMode: "same-origin", sameOriginEntry: "/microapps/timer/index.html", access: "authenticated-member", deckCapability: "none", color: "#7cff2f", icon: "timer", packageName: "planner-timer", apiContracts: [] },
  { id: "assignments", title: "ASSIGNMENTS", subtitle: "Spreadsheet-style tracker", area: "planner", canonicalRoute: "/planner/assignments", launchMode: "same-origin", sameOriginEntry: "/microapps/assignments/index.html", access: "authenticated-member", deckCapability: "none", color: "#ff59c7", icon: "assignments", packageName: "planner-assignments", apiContracts: [] },
]);
...
export function validateAppRegistry(manifests = appManifests) {
  ...
  for (const manifest of manifests) {
    ...
    if (manifest.launchMode === "same-origin") {
      ...
      assertContract(!manifest.legacyTarget && !manifest.legacyFallback, `Same-origin app ${manifest.id} cannot declare a compatibility target.`);
    } else {
...
export function toLauncherCard(manifestOrId) {
...
export function listLauncherCards(area) {
  return appManifests.filter((manifest) => manifest.area === area).map((manifest) => toLauncherCard(manifest));
}

export function listPlatformGames() {
  return appManifests.filter((manifest) => manifest.area === "arcade" && manifest.apiContracts.includes("platformGames")).map((manifest) => {
    const card = toLauncherCard(manifest);
    const route = card.canonicalRoute;
